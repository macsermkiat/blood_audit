...
# Sentinel date used when only a time-of-day is parsed; callers combine the
# parsed time with the row's date column before persisting as UTC.
_SENTINEL_YEAR = 1900
_SENTINEL_MONTH = 1
_SENTINEL_DAY = 1


def _time_only(hour: int, minute: int, second: int) -> datetime:
    return datetime(
        year=_SENTINEL_YEAR,
        month=_SENTINEL_MONTH,
        day=_SENTINEL_DAY,
        hour=hour,
        minute=minute,
        second=second,
...
def parse_hosxp_time(raw: str | None) -> ParseResult:
    ...
    :class:`ParseResult` with either a populated ``value`` and ``parse_warning=None``,
    or ``value=None`` plus a descriptive ``parse_warning``. Never produces both
    or neither (mutual-exclusion invariant covered by the property test suite).
    """
    if raw is None:
    ...
    if len(raw) == 6 and raw.isdigit():
        ...
        if 0 <= h <= 23 and 0 <= m <= 59 and 0 <= s <= 59:
            return ParseResult(value=_time_only(h, m, s), parse_warning=None, raw=raw)
        return ParseResult(
            value=None,
    ...
    if (
        ...
        if 0 <= h <= 23 and 0 <= m <= 59:
            return ParseResult(value=_time_only(h, m, 0), parse_warning=None, raw=raw)
        return ParseResult(
            value=None,
