...
_RBC_INDICATION_VOCABULARY: Final[str] = (
    ...
    "peri-op window). A declared order-time use code (BDVSTDT.USETYPE) alone "
...
_PLATELET_REVIEW_PROMPT: Final[str] = (
    ...
    "  2. Central venous catheter (CVC) insertion, thoracocentesis, "
    "arthrocentesis, or dental extraction: count <50,000 /μL. Bone marrow "
    "aspiration or biopsy needs no threshold and does not ground a transfusion "
    "at any count. Any other invasive procedure not listed here: return "
    "NEEDS_REVIEW rather than applying a threshold of your own.\n"
    "  3. Major non-neuraxial surgery, planned or performed within the "
    "previous 24 h: count <80,000 /μL, or <100,000 /μL if the patient or "
    "procedure carries high bleeding risk.\n"
    ...
    "  6. Intracranial bleed (SDH, EDH, SAH, intracerebral): count "
    "<100,000 /μL when the bleed is acute, acute-on-chronic, documented as "
    "growing or expanding, or neurosurgery is planned (a growing collection "
    "may need emergency surgery and platelets take time to prepare); count "
    "<50,000 /μL when the bleed is stable and non-operative, including a "
    "stable chronic SDH. An acute or acute-on-chronic bleed stays acute for "
    "7 days from the imaging or note that documents it; after 7 days apply "
    "the stable rule unless new imaging or a documented clinical change "
    "shows growth or a new bleed.\n"
    "  7. Clinically significant active bleeding (see the active_bleeding "
    "hard signal for what qualifies): count <50,000 /μL.\n"
    "  8. Aplastic anaemia on active therapy aimed at reversing the "
    "thrombocytopenia (ATG, cyclosporine, eltrombopag, or transplant "
    "work-up), documented in the notes: count <10,000 /μL, OR expected to "
    "drop below 10,000 /μL within 24 hours; during the ATG course or with "
    "sepsis: count <20,000 /μL (BSH aplastic anaemia guideline; clinician "
    "ruling 2026-09-23).\n\n"
    ...
    "  D. Aplastic anaemia that is chronic, stable, or not under active "
    "therapy, without active bleeding (a patient on active therapy is "
    "indication 8, not this line).\n"
    ...
    "indication.\n\n"
    "EXCLUSION OVERRIDE. An exclusion population overrides indications 4 and 5 "
    "however low the count: a count below 10,000 /μL does NOT by itself make "
    "transfusion appropriate in an excluded population. Do not reclassify an "
    "excluded diagnosis as chemo/HSCT or consumptive thrombocytopenia: "
    "immunosuppression for aplastic anaemia (ATG, cyclosporine) is NOT "
    "chemotherapy; it grounds indication 8, which has its own thresholds, "
    "and only when the notes document the active therapy. In an excluded "
    "population APPROPRIATE requires the "
    "condition stated on that population's own line, grounded in the notes. "
    "Where the line names the only acceptable condition (E snakebite, F TTP, "
    "G HIT, H ITP), a procedure indication 1–3 does NOT override it. For the "
    "other excluded populations a procedure indication 1–3 still counts as a "
    "separately-grounded positive indication.\n\n"
    "LOCAL TRIGGERS. A ward standing order or unit protocol trigger written "
    "in the notes (for example 'if plt < 20,000 give platelets') is NOT a "
    "positive indication: it is the practice under audit. Judge the order "
    "against the thresholds above only. For indication 5, 'expected to drop "
    "below 10,000 /μL within 24 hours' must be grounded in the documented "
    "count trend or an explicit clinical note, not in the standing order.\n\n"
    "SPECIALIST TARGET. When a consulting specialist team documented a "
    "platelet target for this patient before the order (for example 'chest "
    "keep plt 100,000'), report that target in /μL in "
    "specialist_platelet_target_per_ul; otherwise report null. This differs "
    "from LOCAL TRIGGERS: a ward standing order or unit protocol is not a "
    "specialist target. The target is not a positive indication: still judge "
    "the order against the policy thresholds above and return the verdict "
    "they give, and quote the specialist's advice in reasoning_summary_en.\n\n"
    "COUNT TREND (indication 5). When the evidence carries a line 'Projected "
    "platelet count in 24 h', that line is the documented count trend for "
    "this clause: it is the straight-line projection through the last two "
    "pre-order counts, which is how the clinical committee defines 'expected "
    "to drop below 10,000 /μL within 24 hours'. If the projected value is "
    "below 10,000 /μL, the clause is met and no further clinical note is "
    "needed. If the projected value is at or above 10,000 /μL, or the line "
    "says the projection is not computable, the clause is not met. Do not "
    "dismiss the projection as a mechanical extrapolation, do not replace it "
    "with your own reading of the counts, and do not let a standing order "
    "stand in for it. Without that line, ground the clause in the documented "
    "count trend or an explicit clinical note as above.\n\n"
    "DIAGNOSIS CODES. An ICD-10 code alone never grounds an indication or an "
    "exclusion population: a code may record an old or a future diagnosis. "
    "The condition must be documented in a note, a lab result or an imaging "
    "report within the audit window; use the code only as context.\n\n"
    ...
    "HARD SIGNALS — report all five booleans in your tool-use response; "
    ...
    "(indications 1–3: LP <50–80k; CVC, thoracocentesis, arthrocentesis, "
    "dental extraction <50k; major surgery <80–100k).\n"
    ...
    "  • intracranial_bleed_indication — indication 6 grounded in the notes "
    "or imaging reports with the count below its threshold. Expansion of a "
    "chronic SDH is NOT active bleeding for this audit: report it here, never "
    "under active_bleeding.\n"
    "  • aplastic_active_therapy_indication — indication 8: aplastic anaemia "
    "on active therapy documented in the notes, with the count below its "
    "indication 8 threshold. Report it here, not under "
    ...
    "qualifying procedure, and no indication 4–8 below its threshold (or an "
