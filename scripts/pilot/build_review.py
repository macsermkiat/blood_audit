...
_REVIEW_REASON_LABELS: dict[str, str] = {
    ...
    "llm_overclear_suspect": (
        ...
        "assert guardrail, plus two live paths — the tool payload is "
        "shape-drifted (missing or garbled indications/negative_evidence), "
        "or a grounded high-confidence ACS citation (a prompt-defined hard "
        "indication the structured system cannot verify) makes asserting "
        "unsafe"
    ),
...
<dt>llm_overclear_suspect</dt><dd>Over-clear floored to NEEDS_REVIEW: historical rows from before the assert guardrail, plus two live paths — the tool payload is shape-drifted (missing or garbled indications/negative_evidence), or a grounded high-confidence ACS citation (a prompt-defined hard indication the structured system cannot verify) makes asserting unsafe.</dd>
